...
AS
    SELECT FirstName + ' ' + LastName AS 'Student Name', Province
	FROM   Student
RETURN
GO
EXEC  Provinces
...
AS
  SELECT  FirstName + ' ' + LastName AS 'Student Name', PT.PaymentTypeDescription
  FROM Student AS S
      LEFT OUTER JOIN Payment AS P ON P.StudentID = S.StudentID
	  LEFT OUTER JOIN PaymentType AS PT ON P.PaymentTypeID = PT.PaymentTypeID
GO
...
AS
  ...
  FROM    Student AS S
      INNER JOIN Payment AS P ON P.StudentID = S.StudentID
